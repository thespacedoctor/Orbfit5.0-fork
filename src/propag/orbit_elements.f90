...
!        CALL filclo(uniin,' ') 
     ELSEIF(form.EQ.'BA2') THEN 
        IF(.NOT.opened) CALL filopn(uniin,infil1,'OLD') 
        CALL rdast2_fork(uniin,infil1,objnam,nobj,deforb,defcn,            &
             &                eltype,telem,elem,cove,nore,mass,h,g,comele)      
        CALL filclo(uniin,' ') 
...
! October 2, 2017: Forked by David Young                                           
...
!                                                                       
SUBROUTINE rdast2_fork(unit,filnam,objnam,nobj,deforb,defcn,           &
     &                  eltype,telem,elem,cove,nore,mass,h,g,comele) 
  USE fund_const   
  IMPLICIT NONE                                                                       
  INTEGER unit,nobj 
  ...
  CHARACTER n1*6,n2*18,name*18,hc*5,gc*5,krc*10,name2*18                                             
  INTEGER lench 
...
10 END DO
  ...
  nr=0 
1 CONTINUE 
  READ(unit,100,END=2) name,name2 
  nr=nr+1 
  IF(name.EQ.'     ') THEN 
     name="NOMATCHPLEASE" 
  END IF
  ...
  DO 3 k=1,nobj 
     IF(deforb(k)) GOTO 3 
     IF(upper(name).EQ.upper(objnam(k))) THEN 
        BACKSPACE(unit) 
...
107     FORMAT(I6) 
     ...
     &              '" at record '//krc(1:lc)  
        nrem=nrem-1 
        IF(nrem.LE.0) RETURN 
     ELSE IF(upper(name2).EQ.upper(objnam(k))) THEN 
        BACKSPACE(unit) 
        READ(unit,103) n1,n2,hc,gc,flags,year,month,day,el1 
103     FORMAT(A6,1X,A18,17X,A5,1X,A5,17X,6I4,12X,I4,2I2,1X,              &
     &       3(F10.6,1X),F9.6,1X,F10.8,1X,F12.8)    
        deforb(k)=.true. 
        defcn(k)=.false. 
        eltype(k)='KEP' 
